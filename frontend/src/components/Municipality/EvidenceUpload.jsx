...
export default function EvidenceUpload({ report, readOnly = false, imageFile, setImageFile, pdfFile, setPdfFile }) {
  ...
  const isFullyUploaded = imageFile && pdfFile;
  ...
  return (
    <div className="bg-[#f1f5f9] shadow-[6px_6px_12px_#cbd5e1,-6px_-6px_12px_#ffffff] border border-white/40 p-5 rounded-2xl space-y-4 text-slate-800">
      <div className="flex items-center justify-between">
        ...
        {isFullyUploaded && <span className="px-2 py-1 bg-emerald-50 text-[#15803D] rounded-md text-[10px] font-bold border border-emerald-100 flex items-center"><ShieldCheck className="w-3.5 h-3.5 mr-1" /> Verified</span>}
      </div>

      {!imageFile && !pdfFile && readOnly ? (
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button 
          ...
          className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center space-y-2 transition ${
            imageFile ? 'border-emerald-350 bg-emerald-50/50 cursor-default' : 'border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 cursor-pointer'
          }`}
        >
          {imageFile ? (
            <>
              ...
              <span className="text-xs font-bold text-emerald-700 truncate w-full px-2">{imageFile.name}</span>
        ...
        />
        <button 
          onClick={() => !readOnly && pdfInputRef.current?.click()}
          className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center space-y-2 transition ${
            pdfFile ? 'border-[#15803D]/20 bg-emerald-50/20 cursor-default' : 'border-slate-300 hover:border-slate-400 bg-white hover:bg-slate-50 cursor-pointer'
          }`}
        >
          {pdfFile ? (
            <>
              ...
              <span className="text-xs font-bold text-emerald-800 truncate w-full px-2">{pdfFile.name}</span>

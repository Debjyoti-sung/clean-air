          ...
          <div className="relative overflow-hidden bg-[#f1f5f9] rounded-[2.5rem] shadow-[12px_12px_24px_#cbd5e1] p-8 md:p-10 space-y-10 animate-fade-in-up">
            ...
            {/* Bottom Area: Chart and Filters */}
              <div className="pt-8 space-y-6 relative">
                ...
                {/* Header: Trend Filters & Legend */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 mt-4">
                  <div className="flex items-center space-x-3 text-slate-800 font-black text-[18px]">
                    <div className="p-2 bg-[#f1f5f9] rounded-lg shadow-[4px_4px_8px_#cbd5e1]">
                      <Activity className="w-5 h-5 text-blue-600" />
                    </div>
                    <span>AQI Trend Analysis</span>
                  </div>

                  {/* Neomorphic Timeframe selector tabs */}
                  <div className="flex items-center space-x-2 bg-[#f1f5f9] p-2 rounded-2xl shadow-[inset_4px_4px_8px_#cbd5e1]">
                    <button
                      onClick={() => { setTimeframe('7d'); setHoveredPoint(null); }}
                      className={`text-[14px] font-bold px-4 py-2 rounded-xl transition-all cursor-pointer ${timeframe === '7d' ? 'bg-[#f1f5f9] shadow-[4px_4px_8px_#cbd5e1] text-emerald-700' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      7 Days
                    </button>
                    <button
                      onClick={() => { setTimeframe('30d'); setHoveredPoint(null); }}
                      className={`text-[14px] font-bold px-4 py-2 rounded-xl transition-all cursor-pointer ${timeframe === '30d' ? 'bg-[#f1f5f9] shadow-[4px_4px_8px_#cbd5e1] text-emerald-700' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                      30 Days
                    </button>
                  </div>
              ...
              <div className="w-full overflow-x-auto relative">
                <div className="min-w-[650px] relative">
                  ...
                  {/* Y-Axis guide levels */}
                  <div className="absolute left-16 right-4 top-[30px] border-b border-dashed border-slate-200 text-slate-400 text-[10px] text-right pr-2 select-none h-0">
                    <span className="absolute left-[-52px] top-[-6px]">300 - Severe</span>
                  </div>
                  <div className="absolute left-16 right-4 top-[85px] border-b border-dashed border-slate-200 text-slate-400 text-[10px] text-right pr-2 select-none h-0">
                    <span className="absolute left-[-52px] top-[-6px]">200 - V. Poor</span>
                  </div>
                  <div className="absolute left-16 right-4 top-[140px] border-b border-dashed border-slate-200 text-slate-400 text-[10px] text-right pr-2 select-none h-0">
                    <span className="absolute left-[-52px] top-[-6px]">100 - Moderate</span>
                  </div>
                  <div className="absolute left-16 right-4 top-[195px] border-b border-dashed border-slate-200 text-slate-400 text-[10px] text-right pr-2 select-none h-0">
                    <span className="absolute left-[-52px] top-[-6px]">50 - Good</span>
                  </div>

                  <svg className="w-full h-[240px] z-10 relative overflow-visible" viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
                    <defs>
                    ...
                    {coordinates.map((p, index) => {
                      const colorMeta = getAqiCategory(p.aqi);
                      ...
                      return (
                        <g key={index} className="cursor-pointer">
                          <circle
                            ...
                            r="16"
                            fill="transparent"
                            onMouseEnter={() => setHoveredPoint({ ...p, index, hex: colorMeta.hex, cat: colorMeta.name })}
                  ...
                  {hoveredPoint && (
                    <div
                      className="absolute bg-slate-900 text-white rounded-lg p-2.5 shadow-xl border border-slate-700 pointer-events-none z-20 flex flex-col space-y-0.5 text-left text-[11px] min-w-[120px] transition-all duration-100"
                      style={{
                    ...
                    >
                      <div className="font-bold border-b border-slate-800 pb-1 flex justify-between items-center space-x-2">
                        <span className="text-slate-300">{hoveredPoint.label}</span>
                        <span className="px-1.5 py-0.2 rounded font-extrabold" style={{ color: hoveredPoint.hex }}>
                          AQI {hoveredPoint.aqi}
                        </span>
                      </div>
                      <div className="pt-1 flex flex-col space-y-0.5 text-slate-400">
                        <span>Status: <strong style={{ color: hoveredPoint.hex }}>{hoveredPoint.cat}</strong></span>
                        <span>Temp: <strong>{hoveredPoint.temp}</strong></span>
                        <span>Wind: <strong>{hoveredPoint.wind}</strong></span>
                      </div>
                    </div>
                  )}
              ...
              {/* Bottom info */}
              <div className="flex justify-between items-center text-[12px] text-slate-400 font-medium pt-2 border-t border-slate-100">
                <span className="flex items-center space-x-1">
                  <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
                  <span>Automatic telemetry sync active</span>
                </span>
                <span>Last evaluated: Just Now (IST)</span>
              </div>

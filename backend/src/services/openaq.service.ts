...
import { AirQuality, Pollutants } from '../types/map.types.js';

export const OpenAQService = {
  /**
   * Fetch latest Air Quality stats from nearest OpenAQ station
   */
  getAirQuality: async (lat: number, lng: number): Promise<AirQuality> => {
...
function getMockAirQuality(lat: number, lng: number): AirQuality {
